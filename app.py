...
def main():
    ...
    try:
        ...
        for route in routes_to_process:
            with st.spinner(f'Carregando dados para {route}...'):
                try:
                    ...
                    if error or raw_df.empty:
                        raise ValueError(f"Dados não encontrados para {route}")
                    ...
                    # Carregar alertas uma única vez
                    if alerts_df.empty:
                    ...
                    st.error(f"Erro na rota {route}: {str(e)}")
                    continue
        ...
        st.header("🗺️ Visualização Integrada")
        ...
        # --- Seções de Análise ---
        # (Manter o restante do código de análise aqui)
        # ... [Seções de análise preditiva, alertas e detalhes técnicos] ...
